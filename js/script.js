$(document).ready(function(){
  ...
  $('#search').keyup(function(){
      ...
      let htmlElement = (value, key) => {
	    return  '<li class="list-group-item link-class"><img src="'+value.picture.thumbnail +'" height="40" width="40" class="img-thumbnail" /> '+value.name.first+" "+value.name.last+' | <span class="text-muted">Adress: '+value.location.street+'</span> | <span class="text-muted">Age: '
        ...
        $.each(data.results, function(key, value){
          ...
          if('name' == checkedRadio){
            ...
            if (value.name.first.search(expression) != -1 || value.name.last.search(expression) != -1 ){
              // Append content
              $('#result').append(htmlElement(value, key));
